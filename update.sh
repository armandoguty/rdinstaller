...
echo " "
echo -e "${BLUE}You have to ${RED}MANUALLY${BLUE} check the corresponding repository (${NC}https://github.com/armandoguty/rustdesk-api-server${BLUE}) for relevant changes and update it (including the client config scripts) using ${NC}git pull${BLUE} within ${NC}/opt/rustdesk-api-server${BLUE}.${NC}"
echo " "
echo "To update rustdesk-api-server"
echo "First backup old folder"
echo "sudo cp -r /opt/rustdesk-api-server /opt/rustdesk-api-server-BK"
echo "Then navigate to the opt folder and then update the folder"
echo "cd /opt/rustdesk-api-server"
echo "sudo git pull https://github.com/armandoguty/rustdesk-api-server.git"
